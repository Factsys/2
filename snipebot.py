...
# Bot owner IDs (comma-separated string that gets parsed into a set)
BOT_OWNER_IDS_STR = "1334138321412296725"
BOT_OWNER_IDS = set(
    int(id.strip()) for id in BOT_OWNER_IDS_STR.split(",") if id.strip())
...
def find_user_by_name(guild, search_term):
    ...
    for member in guild.members:
        if member.display_name.lower() == search_term or member.name.lower(
        ) == search_term:
            return member
    ...
    if matches:
        names = [m.display_name.lower()
                 for m in matches] + [m.name.lower() for m in matches]
        closest = difflib.get_close_matches(search_term,
                                            names,
                                            n=1,
                                            cutoff=0.3)
        if closest:
            ...
            for member in matches:
                if member.display_name.lower(
                ) == closest_name or member.name.lower() == closest_name:
                    return member
        return matches[0]
...
channel_webhooks = {}
namelocked_users = {
}  # Format: {user_id: {'guild_id': guild_id, 'nickname': 'locked_nickname'}}
...
def detect_media_type(url):
    ...
    if 'tenor.com' in url_lower or 'tenor.co' in url_lower:
        ...
        return "reddit_media"
    elif 'discord' in url_lower and ('cdn.discord' in url_lower
                                     or 'media.discord' in url_lower):
...
def get_media_url(content, attachments):
    ...
    if attachments:
        for attachment in attachments:
            logger.info(
                f"Found attachment: {attachment.filename} - {attachment.url}")
            media_urls.append({
                'url': attachment.url,
    ...
    if content:
        # Tenor detection
        tenor_matches = re.finditer(
            r'https?://(?:www\.)?tenor\.com/view/[^\s]+', content)
        ...
        # Giphy detection
        giphy_matches = re.finditer(
            r'https?://(?:www\.)?giphy\.com/gifs/[^\s]+', content)
        ...
        discord_matches = re.finditer(
            r'https?://(?:cdn|media)\.discordapp\.(?:com|net)/[^\s]+\.(gif|png|jpg|jpeg|webp|mp4|mov)[^\s]*',
            content)
        ...
        direct_matches = re.finditer(
            r'https?://[^\s]+\.(gif|png|jpg|jpeg|webp|mp4|mov|mp3|wav|ogg)[^\s]*',
            content)
        ...
        # Twitter/X detection
        twitter_matches = re.finditer(
            r'https?://(?:www\.)?(?:twitter\.com|x\.com)/[^\s]+', content)
...
def check_giveaway_requirements(member, requirements):
    ...
    if 'messages' in requirements:
        ...
        if user_count < required_messages:
            failed_requirements.append(
                f"Need {required_messages} messages (has {user_count})")
    ...
    if 'time_in_server' in requirements:
        ...
        if join_time:
            ...
            if time_in_server < required_time:
                ...
                failed_requirements.append(
                    f"Need {required_str} in server (has {current_str})")

    if 'required_role' in requirements:
        role_name = requirements['required_role']
        if not any(role.name.lower() == role_name.lower()
    ...
    if 'blacklisted_role' in requirements:
        ...
        if any(role.name.lower() == role_name.lower()
               for role in member.roles):
...
class RemoveUserModal(discord.ui.Modal, title="Remove Participant"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not target_user or user_id not in giveaway['participants']:
            await interaction.response.send_message(
                "❌ User not found in giveaway participants.", ephemeral=True)
            return

        # Remove user from participants
        giveaway['participants'].remove(user_id)
        await interaction.response.send_message(
            f"✅ Removed **{target_user.name}** from the giveaway.",
            ephemeral=True)
...
class GiveawayView(discord.ui.View):
    ...
    async def join_giveaway(self, interaction: discord.Interaction,
        ...
        if is_user_blocked(interaction.user.id):
            await interaction.response.send_message(
                "❌ You are blocked from using bot functions.", ephemeral=True)
            return
        ...
        if 'requirements' in giveaway and giveaway['requirements']:
            ...
            if guild_member:
                meets_requirements, failed_reqs = check_giveaway_requirements(
                    guild_member, giveaway['requirements'])
                if not meets_requirements:
                    failed_text = "\n".join(
                        [f"• {req}" for req in failed_reqs])
                    await interaction.response.send_message(
                        f"❌ **You don't meet the requirements:**\n{failed_text}",
                        ephemeral=True)
                    return
        ...
        await interaction.response.send_message(
            "✅ You have successfully joined the giveaway!", ephemeral=True)

    @discord.ui.button(label="List",
                       style=discord.ButtonStyle.secondary,
                       emoji="📋")
    async def list_participants(self, interaction: discord.Interaction,
                                button: discord.ui.Button):
        if is_user_blocked(interaction.user.id):
            await interaction.response.send_message(
                "❌ You are blocked from using bot functions.", ephemeral=True)
        ...
        for page in range(total_pages):
            ...
            end_idx = min((page + 1) * participants_per_page,
                          len(participants))
            ...
            participant_list = []
            for i, user_id in enumerate(page_participants,
                                        start=start_idx + 1):
                user = bot.get_user(user_id)
                if user:
                    participant_list.append(
                        f"{i}. {user.mention} ({user.name})")
                else:
        ...
        if (is_bot_owner(interaction.user.id)
                ...
                or can_host_giveaway(interaction.user)):
...
class RerollView(discord.ui.View):
    ...
    @discord.ui.button(label="Reroll",
    ...
    async def reroll_button(self, interaction: discord.Interaction,
        ...
        if not participants:
            await interaction.response.send_message(
                "❌ No participants to reroll.", ephemeral=True)
            return
        ...
        if new_winner:
            embed = discord.Embed(
                ...
                description=
                f"**🎊 Congratulations {new_winner.mention}!**\nYou won **{self.giveaway_data['prize']}**!",
                color=discord.Color.green())
            embed.set_footer(text=f"Rerolled by {interaction.user.name}")

            # Send winner notification
            try:
                await new_winner.send(
                    f"🎉 **Congratulations!** You won **{self.giveaway_data['prize']}** in {interaction.guild.name}!"
                )
            except:
                pass
            ...
            await interaction.response.send_message(
                "❌ Could not find the new winner.", ephemeral=True)
    ...
    async def role_button_callback(self, interaction: discord.Interaction):
        ...
        try:
            if role in member.roles:
                ...
                await interaction.response.send_message(
                    f"✅ Added role **{role.name}**", ephemeral=True)
...
# --- Updated Help Views ---
class MemberHelpPaginationView(discord.ui.View):
    ...
    def __init__(self, timeout=300):
        ...
        self.pages = [{
            "title":
            "📜 FACTSY Member Commands - Page 1",
            "fields":
            [("**Message Tracking**",
              ",snipe <n> — Show deleted message by number\n,sp <#channel> <page> — List normal deleted messages\n,spl <#channel> <page> — Show deleted links only",
              False),
             ("**Info**",
              ",help < > — Show this help\n,ping — Show bot latency", False)]
        }, {
            "title":
            "📜 FACTSY Member Commands - Page 2",
            "fields":
            [("**Fun/Utility**",
              ",editsnipe — Show last edited message\n,spa < > — All deleted messages (all channels)\n,spla < > — All deleted links (all channels)",
              False), ("**Usage**", "Use < > to go through pages.", False)]
        }]
    ...
    def get_embed(self):
        ...
        embed = discord.Embed(title=page_data["title"],
                              color=discord.Color.blue())
        for name, value, inline in page_data["fields"]:
            embed.add_field(name=name, value=value, inline=inline)
        embed.set_footer(
            text=
            f"Page {self.current_page + 1} of {self.total_pages} | Use < > to go through pages"
        )
        return embed
...
class ModHelpPaginationView(discord.ui.View):
    ...
    def __init__(self, timeout=300):
        ...
        self.pages = [{
            "title":
            "🛡️ FACTSY Mod Commands - Page 1",
            "fields":
            [("**Moderation**",
              ",namelock <@user> <name>\n,unl <@user>\n,rename <@user> <name>\n,block <@user>\n,role <@user> <@role>\n,namelockimmune <@user>",
              False),
             ("**Giveaways**",
              ",giveaway <...>\n,gw <id>\n,giveaway_host <@role>", False)]
        }, {
            "title":
            "🛡️ FACTSY Mod Commands - Page 2",
            "fields":
            [("**Management**",
              ",manage\n,perm <@user>\n,permess <@user>\n,ms <@role>", False),
             ("**Utility**",
              ",say <msg>\n,saywb <#channel> <title> <desc> <color>\n,create <title> <color> <emoji> <role> ...",
              False)]
        }, {
            "title":
            "🛡️ FACTSY Mod Commands - Page 3",
            "fields":
            [("**Tracking**",
              ",spfa < >\n,spf <#channel> <page>\n,sp <#channel> <page>\n,spa < >\n,spla < >\n,editsnipe\n,snipe <n>",
              False),
             ("**Info**",
              ",helpm < > — Show this help\n,ping\n,prefix <new>\n,help < >",
              False)]
        }]
        self.total_pages = len(self.pages)
        self.update_buttons()
...
@bot.tree.command(name="helpm",
                  description="Show mod help (mods/admins/owners only)")
@check_not_blocked()
async def helpm_slash(interaction: discord.Interaction):
    ...
    if not (interaction.user.guild_permissions.manage_messages
            or interaction.user.guild_permissions.administrator
            or is_bot_owner(interaction.user.id)):
        await interaction.response.send_message(
            "❌ Only mods/admins/owners can use this command!", ephemeral=True)
        return
    view = ModHelpPaginationView()
    embed = view.get_embed()
    await interaction.response.send_message(embed=embed, view=view)
...
async def on_message_delete(message):
    ...
    cleaned_content = clean_content_from_media(
        message.content, media_urls) if media_urls else message.content
    ...
    if len(sniped_messages[channel_id]) > MAX_MESSAGES:
        sniped_messages[channel_id] = sniped_messages[
...
async def on_message_edit(before, after):
    ...
    if len(edited_messages[channel_id]) > MAX_MESSAGES:
        edited_messages[channel_id] = edited_messages[
            channel_id][:MAX_MESSAGES]
            ...
            if after.nick != locked_nickname:
                try:
                    await after.edit(nick=locked_nickname,
                                     reason="Nickname locked by FACTSY")
                    print(
                        f"Reverted nickname for {after.name} to {locked_nickname}"
                    )
                except discord.Forbidden:
                    print(
                        f"Cannot revert nickname for {after.name} - insufficient permissions"
                    )
                except Exception as e:
        ...
        if namelock_data['guild_id'] == member.guild.id:
            ...
            try:
                await member.edit(nick=locked_nickname,
                                  reason="Re-applying namelock on rejoin")
                print(
                    f"Re-applied namelock for {member.name} to {locked_nickname}"
                )
            except discord.Forbidden:
                print(
                    f"Cannot re-apply namelock for {member.name} - insufficient permissions"
                )
            except Exception as e:
            ...
            if winner_user:
                ...
                try:
                    await winner_user.send(
                        f"🎉 **Congratulations!** You won **{giveaway['prize']}** in {channel.guild.name}!"
                    )
                except:
                    pass

        winner_text = ", ".join(
            winner_mentions) if winner_mentions else "Unknown users"
...
async def snipe_command(ctx, number: int = 1):
    ...
    if number < 1 or number > MAX_MESSAGES:
        await ctx.send(
            f"❌ Please provide a number between 1 and {MAX_MESSAGES}")
        return
    ...
    if number > len(sniped_messages[channel_id]):
        await ctx.send(
            f"❌ Only {len(sniped_messages[channel_id])} deleted messages available!"
        )
        return
...
async def editsnipe_command(ctx):
    ...
    embed.add_field(name="Before",
                    value=edited_msg['before_content'][:1024]
                    if edited_msg['before_content'] else "*No content*",
                    inline=False)
    embed.add_field(name="After",
                    value=edited_msg['after_content'][:1024]
                    if edited_msg['after_content'] else "*No content*",
                    inline=False)
    embed.set_author(name=edited_msg['author'].display_name,
                     icon_url=edited_msg['author'].display_avatar.url)
...
@bot.command(name='sp')
@not_blocked()
async def snipe_pages_command(ctx, channel: str = None, page: int = 1):
    target_channel = find_channel_robust(ctx.guild,
                                         channel) if channel else ctx.channel
    if not target_channel:
        await ctx.send(f"❌ Channel `{channel}` not found or ambiguous!")
        return
    ...
    if channel_id not in sniped_messages or not sniped_messages[channel_id]:
        ...
        return
    normal_messages = [
        msg for msg in sniped_messages[channel_id]
    ...
    for i, msg in enumerate(page_messages, start=start_idx + 1):
        ...
        embed.add_field(name=f"{i}. {msg['author'].mention}",
                        value=f"{msg['author'].mention}\n{content}",
                        inline=False)
    embed.set_footer(
        text=
        f"Page {page} of {total_pages} | {len(normal_messages)} total messages"
    )
    if total_pages > 1:
        embeds = []
        for p in range(1, total_pages + 1):
            p_start_idx = (p - 1) * MESSAGES_PER_PAGE
            p_end_idx = min(p_start_idx + MESSAGES_PER_PAGE,
                            len(normal_messages))
            ...
            if p_page_messages:
                ...
                p_embed.set_author(name=user.display_name,
                                   icon_url=user.display_avatar.url)
            for i, msg in enumerate(p_page_messages, start=p_start_idx + 1):
                content = msg['content'] or "*No text content*"
                if is_offensive_content(msg['content']):
                    content = filter_content(msg['content'])
                p_embed.add_field(name=f"{i}. {msg['author'].mention}",
                                  value=f"{msg['author'].mention}\n{content}",
                                  inline=False)
            p_embed.set_footer(
                text=
                f"Page {p} of {total_pages} | {len(normal_messages)} total messages"
            )
...
@not_blocked()
async def snipe_filtered_command(ctx, channel: str = None, page: int = 1):
    target_channel = find_channel_robust(ctx.guild,
                                         channel) if channel else ctx.channel
    if not target_channel:
        await ctx.send(f"❌ Channel `{channel}` not found or ambiguous!")
    ...
    embed.set_footer(
        text=
        f"Page {page} of {total_pages} | {len(filtered_messages)} total messages"
    )
    if total_pages > 1:
        embeds = []
        for p in range(1, total_pages + 1):
            ...
            p_embed.set_footer(
                text=
                f"Page {p} of {total_pages} | {len(filtered_messages)} total messages"
            )
            embeds.append(p_embed)
        view = PaginationView(embeds)
        view.current_page = page - 1
...
@not_blocked()
async def snipe_links_command(ctx, channel: str = None, page: int = 1):
    target_channel = find_channel_robust(ctx.guild,
                                         channel) if channel else ctx.channel
    if not target_channel:
        await ctx.send(f"❌ Channel `{channel}` not found or ambiguous!")
    ...
    embed.set_footer(
        text=
        f"Page {page} of {total_pages} | {len(link_messages)} total messages")
    if total_pages > 1:
        embeds = []
        for p in range(1, total_pages + 1):
            p_start_idx = (p - 1) * MESSAGES_PER_PAGE
            ...
            p_embed.set_footer(
                text=
                f"Page {p} of {total_pages} | {len(link_messages)} total messages"
            )
            embeds.append(p_embed)
        view = PaginationView(embeds)
        view.current_page = page - 1
...
async def snipe_all_command(ctx, page: int = 1):
    ...
    if not all_msgs:
        await ctx.send(
            "❌ No normal deleted messages found in any channel/thread!")
        return
    ...
    for i, (msg, channel_id) in enumerate(page_msgs, start=start_idx + 1):
        ...
        embed.add_field(
            ...
            inline=False)
    embed.set_footer(
        text=f"Page {page} of {total_pages} | {len(all_msgs)} total messages")
    if total_pages > 1:
        embeds = []
        for p in range(1, total_pages + 1):
            p_start_idx = (p - 1) * MESSAGES_PER_PAGE
            ...
            if p_page_msgs:
                ...
                p_embed.set_author(name=user.display_name,
                                   icon_url=user.display_avatar.url)
            for i, (msg, channel_id) in enumerate(p_page_msgs,
                                                  start=p_start_idx + 1):
                content = msg['content'] or "*No text content*"
                if is_offensive_content(msg['content']):
                    content = filter_content(msg['content'])
                ...
                p_embed.add_field(
                    name=
                    f"{i}. {user.display_name} ({user.id}) in #{channel_name}",
                    value=f"{user.mention}\n{content}",
                    inline=False)
            p_embed.set_footer(
                text=
                f"Page {p} of {total_pages} | {len(all_msgs)} total messages")
            embeds.append(p_embed)
        view = PaginationView(embeds)
        view.current_page = page - 1
        view.update_buttons()
...
async def snipe_all_filtered_command(ctx, page: int = 1):
    ...
    if not all_msgs:
        await ctx.send(
            "❌ No filtered deleted messages found in any channel/thread!")
        return
    all_msgs.sort(key=lambda x: x[0]['time'], reverse=True)
    total_pages = math.ceil(len(all_msgs) / MESSAGES_PER_PAGE)
    if page < 1 or page > total_pages:
...
async def snipe_all_links_command(ctx, page: int = 1):
    ...
    if not all_msgs:
        await ctx.send(
            "❌ No deleted link messages found in any channel/thread!")
        return
    all_msgs.sort(key=lambda x: x[0]['time'], reverse=True)
    total_pages = math.ceil(len(all_msgs) / MESSAGES_PER_PAGE)
    if page < 1 or page > total_pages:
...
async def namelock_command(ctx, member: discord.Member, *, nickname):
    ...
    try:
        ...
        await ctx.send(
            f"✅ **{member.display_name}** has been namelocked to `{nickname}`")
...
async def nameunlock_command(ctx, member: discord.Member):
    ...
    await ctx.send(
        f"✅ **{member.display_name}** has been unlocked from namelock")
...
@not_blocked()
async def create_reaction_roles(ctx,
                                title: str,
                                color: str = None,
                                *role_mappings):
    ...
    if len(role_mappings) < 2 or len(role_mappings) % 2 != 0:
        await ctx.send(
            "❌ Please provide emoji-role pairs! Format: `/create [title] [color] [emoji1] [role1] [emoji2] [role2]...`"
        )
        return
    ...
    embed.add_field(name="Available Roles",
                    value="\n".join(role_info),
                    inline=False)
...
async def block_user_command(ctx, user: discord.User):
    ...
    if user.id in BOT_OWNER_IDS:
        await ctx.send("❌ Cannot block a bot owner!")
        return
...
async def giveaway_reroll_command(ctx, message_id: int):
    ...
    if not (is_bot_owner(ctx.author.id)
            or ctx.author.guild_permissions.administrator
            or can_host_giveaway(ctx.author)):
    ...
    try:
        ...
        await ctx.send(
            "🔄 Use the Reroll button on the giveaway message to reroll winners!"
        )
    except discord.NotFound:
...
async def message_user_command(ctx, user_search: str, *, message):
    ...
    if is_bot_owner(ctx.author.id):
        allowed = True
    elif ctx.author.id in mess_permitted_users:
        allowed = True
    elif ctx.guild and ctx.guild.id in mess_permitted_roles:
        user_role_ids = [role.id for role in ctx.author.roles]
        if any(rid in mess_permitted_roles[ctx.guild.id]
               for rid in user_role_ids):
            allowed = True
    if not allowed:
        await ctx.send(
            "❌ Only the bot owner or permitted users/roles can use this command!"
        )
        return
...
@not_blocked()
async def add_role_command(ctx, member_search: str, *, role_search: str):
    """Add a role to a user (robust lookup for user and role)"""
    if not (ctx.author.guild_permissions.manage_roles or is_bot_owner(
            ctx.author.id) or ctx.author.id in permitted_users):
        await ctx.send("❌ You don't have permission to manage roles!")
        return
    member = find_member_robust(ctx.guild, member_search)
    if not member:
        await ctx.send(f"❌ User `{member_search}` not found or ambiguous!")
        return
    role = find_role_robust(ctx.guild, role_search)
    if not role:
        await ctx.send(f"❌ Role `{role_search}` not found or ambiguous!")
        return
    if ctx.author.top_role <= role and not is_bot_owner(ctx.author.id):
        await ctx.send(
            "❌ You can only give roles lower than your highest role!")
        return
    try:
...
async def namelock_immune_command(ctx, member: discord.Member):
    ...
    if member.id in namelock_immune_users:
        ...
        await ctx.send(
            f"✅ **{member.display_name}** is no longer immune to namelock")
    else:
...
async def manage_command(ctx):
    ...
    embed = discord.Embed(title="🔧 Bot Management Panel",
                          color=discord.Color.blue())
    embed.add_field(
        name="📊 Statistics",
        value=
        ...
        inline=False)
    embed.add_field(name="🚫 Blocked Users",
                    value=f"{len(blocked_users)} users",
                    inline=True)
    embed.add_field(name="🔒 Namelocked Users",
                    value=f"{len(namelocked_users)} users",
                    inline=True)
    embed.add_field(name="🎉 Active Giveaways",
                    value=f"{len(active_giveaways)} giveaways",
                    inline=True)
    await ctx.send(embed=embed)
...
@bot.tree.command(name="sp",
                  description="Show paginated list of normal deleted messages")
@check_not_blocked()
async def sp_slash(interaction: discord.Interaction,
    ...
    if channel_id not in sniped_messages or not sniped_messages[channel_id]:
        await interaction.followup.send(
            "❌ No deleted messages found in that channel!")
        return
    ...
    for p in range(1, total_pages + 1):
        ...
        for i, msg in enumerate(p_page_messages, start=p_start_idx + 1):
            ...
            if is_offensive_content(msg['content']):
                content = filter_content(msg['content'])
            p_embed.add_field(name=f"{i}. {msg['author'].display_name}",
                              value=f"{msg['author'].display_name}\n{content}",
                              inline=False)
...
@bot.tree.command(
    name="spf", description="Show paginated list of filtered deleted messages")
@check_not_blocked()
async def spf_slash(interaction: discord.Interaction,
                    channel: discord.TextChannel = None,
                    page: int = 1):
...
@bot.tree.command(name="spl",
                  description="Show paginated list of deleted link messages")
@check_not_blocked()
async def spl_slash(interaction: discord.Interaction,
                    channel: discord.TextChannel = None,
                    page: int = 1):
...
@check_not_blocked()
async def namelock_slash(interaction: discord.Interaction,
    ...
    if member.id in namelock_immune_users:
        await interaction.response.send_message(
            "❌ This user is immune to namelock!", ephemeral=True)
        return

    if member.id == interaction.guild.owner_id:
        await interaction.response.send_message(
            "❌ Cannot namelock the server owner!", ephemeral=True)
        return

    # FIXED: Use the actual nickname parameter instead of hardcoded "test"
    try:
        await member.edit(nick=nickname,
                          reason=f"Namelocked by {interaction.user}")
...
@check_not_blocked()
async def nameunlock_slash(interaction: discord.Interaction,
    ...
    if member.id not in namelocked_users:
        await interaction.response.send_message(
            "❌ This user is not namelocked!", ephemeral=True)
        return
...
@check_not_blocked()
async def rename_slash(interaction: discord.Interaction,
                       member: discord.Member,
                       nickname: str = None):
    """Change a user's nickname"""
    if not (interaction.user.guild_permissions.manage_nicknames
    ...
    try:
        ...
        await interaction.response.send_message(
            f"✅ Renamed **{old_nick}** to **{new_nick}**")
...
@bot.tree.command(name="saywb",
                  description="Send an embed message to a channel")
@check_not_blocked()
async def saywb_slash(interaction: discord.Interaction,
                      channel: discord.TextChannel,
                      color: str = None,
                      title: str = None,
                      description: str = None):
...
@bot.tree.command(name="create",
                  description="Create reaction roles with title")
@check_not_blocked()
async def create_slash(interaction: discord.Interaction,
    ...
    if not role_mappings:
        await interaction.response.send_message(
            "❌ Please provide at least one emoji-role pair!", ephemeral=True)
        return
...
async def block_slash(interaction: discord.Interaction, user: discord.User):
    ...
    if user.id in BOT_OWNER_IDS:
        await interaction.response.send_message("❌ Cannot block a bot owner!",
                                                ephemeral=True)
        return

    if user.id in blocked_users:
        await interaction.response.send_message("❌ User is already blocked!",
                                                ephemeral=True)
        return

    blocked_users.add(user.id)
    await interaction.response.send_message(
        f"✅ **{user.name}** has been blocked from using the bot")
...
async def unblock_slash(interaction: discord.Interaction, user: discord.User):
    ...
    if user.id not in blocked_users:
        await interaction.response.send_message("❌ User is not blocked!",
                                                ephemeral=True)
        return
...
@bot.tree.command(name="giveaway",
                  description="Create a giveaway with advanced requirements")
@check_not_blocked()
async def giveaway_slash(interaction: discord.Interaction,
    ...
    if duration_seconds == 0:
        await interaction.response.send_message(
            "❌ Invalid duration! Use format like: 10s, 5m, 1h, 2d",
            ephemeral=True)
        return

    if duration_seconds < 10:
        await interaction.response.send_message(
            "❌ Minimum giveaway duration is 10 seconds!", ephemeral=True)
        return

    if winners < 1 or winners > 20:
        await interaction.response.send_message(
            "❌ Number of winners must be between 1 and 20!", ephemeral=True)
        return
    ...
    if role_req:
        resolved_role = find_role_robust(interaction.guild, role_req)
        if not resolved_role:
            await interaction.response.send_message(
                f"❌ Required role `{role_req}` not found or ambiguous!",
                ephemeral=True)
            return
        requirements['required_role'] = resolved_role.name

    if blacklisted_role:
        resolved_blacklist = find_role_robust(interaction.guild,
                                              blacklisted_role)
        if not resolved_blacklist:
            await interaction.response.send_message(
                f"❌ Blacklisted role `{blacklisted_role}` not found or ambiguous!",
                ephemeral=True)
            return
        requirements['blacklisted_role'] = resolved_blacklist.name
    ...
    if requirements:
        ...
        if 'required_role' in requirements:
            req_text.append(
                f"• Must have role: {requirements['required_role']}")
        if 'blacklisted_role' in requirements:
            req_text.append(
                f"• Cannot have role: {requirements['blacklisted_role']}")
    ...
    embed.add_field(name="How to Join",
                    value="Click Join to participate!",
                    inline=False)
    ...
    await interaction.response.send_message("✅ Creating giveaway...",
                                            ephemeral=True)
...
@bot.tree.command(name="giveaway_host",
                  description="Set roles that can host giveaways")
@check_not_blocked()
async def giveaway_host_slash(interaction: discord.Interaction,
                              role: discord.Role):
    ...
    if role.id in giveaway_host_roles[guild_id]:
        ...
        await interaction.response.send_message(
            f"✅ Removed **{role.name}** from giveaway host roles")
    else:
        giveaway_host_roles[guild_id].add(role.id)
        await interaction.response.send_message(
            f"✅ Added **{role.name}** to giveaway host roles")
...
@check_not_blocked()
async def mess_slash(interaction: discord.Interaction, user: discord.User,
                     message: str):
    """Send a DM to a user globally"""
    allowed = False
    if is_bot_owner(interaction.user.id):
        ...
        if any(rid in mess_permitted_roles[interaction.guild.id]
               for rid in user_role_ids):
            allowed = True
    if not allowed:
        await interaction.response.send_message(
    ...
    try:
        ...
        await interaction.response.send_message(
            f"✅ Message sent to **{user.name}**", ephemeral=True)
    except discord.Forbidden:
        await interaction.response.send_message(
            "❌ Cannot send DM to this user!", ephemeral=True)
    except Exception as e:
        await interaction.response.send_message(
            f"❌ An error occurred: {str(e)}", ephemeral=True)
...
@check_not_blocked()
async def role_slash(interaction: discord.Interaction, member: discord.Member,
                     role: str):
    if not (interaction.user.guild_permissions.manage_roles
            or is_bot_owner(interaction.user.id)):
        await interaction.response.send_message(
            "❌ You don't have permission to manage roles!", ephemeral=True)
        return
    resolved_role = find_role_robust(interaction.guild, role)
    if not resolved_role:
        await interaction.response.send_message(
            f"❌ Role `{role}` not found or ambiguous!", ephemeral=True)
        return
    try:
        await member.add_roles(resolved_role,
                               reason=f"Added by {interaction.user}")
        await interaction.response.send_message(
            f"✅ Added role **{resolved_role.name}** to **{member.display_name}**"
        )
    except discord.Forbidden:
        await interaction.response.send_message(
            "❌ I don't have permission to manage this role!", ephemeral=True)
    except Exception as e:
        await interaction.response.send_message(
            f"❌ An error occurred: {str(e)}", ephemeral=True)


@bot.tree.command(name="namelockimmune",
                  description="Make a user immune to namelock")
@check_not_blocked()
async def namelock_immune_slash(interaction: discord.Interaction,
                                member: discord.Member):
    """Make a user immune to namelock"""
    if not is_bot_owner(interaction.user.id):
        await interaction.response.send_message(
    ...
    if member.id in namelock_immune_users:
        ...
        namelock_immune_users.add(member.id)
        await interaction.response.send_message(
            f"✅ **{member.display_name}** is now immune to namelock",
            ephemeral=True)
...
async def prefix_slash(interaction: discord.Interaction, new_prefix: str):
    ...
    if len(new_prefix) > 5:
        await interaction.response.send_message(
            "❌ Prefix cannot be longer than 5 characters!", ephemeral=True)
        return
...
@bot.tree.command(
    name="perm",
    description="Toggle full command access for a user (owner only)")
async def perm_slash(interaction: discord.Interaction, user: discord.User):
    ...
    if user.id in permitted_users:
        ...
        await interaction.response.send_message(
            f"✅ Removed full command access from {user.mention}",
            ephemeral=True)
    else:
        permitted_users.add(user.id)
        await interaction.response.send_message(
            f"✅ Granted full command access to {user.mention}", ephemeral=True)


# Add sets for mess command permissions
mess_permitted_users = set()  # user IDs
mess_permitted_roles = {}  # {guild_id: set(role_ids)}


@bot.tree.command(name="permess",
                  description="Toggle access to ,mess for a user")
async def permess_slash(interaction: discord.Interaction, user: discord.User):
    if not is_bot_owner(interaction.user.id):
        await interaction.response.send_message(
            "❌ Only the bot owner can use this command!", ephemeral=True)
        return
    if user.id in mess_permitted_users:
        mess_permitted_users.remove(user.id)
        await interaction.response.send_message(
            f"✅ Revoked ,mess access from {user.mention}", ephemeral=True)
    else:
        mess_permitted_users.add(user.id)
        await interaction.response.send_message(
            f"✅ Granted ,mess access to {user.mention}", ephemeral=True)


@bot.tree.command(
    name="ms", description="Toggle access to ,mess for a role in this server")
async def ms_slash(interaction: discord.Interaction, role: discord.Role):
    if not is_bot_owner(interaction.user.id):
        await interaction.response.send_message(
            "❌ Only the bot owner can use this command!", ephemeral=True)
        return
    guild_id = interaction.guild.id
    if guild_id not in mess_permitted_roles:
        mess_permitted_roles[guild_id] = set()
    if role.id in mess_permitted_roles[guild_id]:
        mess_permitted_roles[guild_id].remove(role.id)
        await interaction.response.send_message(
            f"✅ Revoked ,mess access from {role.mention}", ephemeral=True)
    else:
        mess_permitted_roles[guild_id].add(role.id)
        await interaction.response.send_message(
            f"✅ Granted ,mess access to {role.mention}", ephemeral=True)
...
def find_member_robust(guild, search_term):
    ...
    matches = [
        m for m in guild.members if search_term in m.display_name.lower()
    ...
    for m in guild.members:
        if m.display_name.lower() == search_term or m.name.lower(
        ) == search_term:
            return m
    return None
